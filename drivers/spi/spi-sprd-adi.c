...
struct sprd_adi_data {
	...
	void (*restart)(struct restart_data *data);
	void (*wdg_rst)(void *p);
};
...
static void sprd_adi_restart(struct sprd_adi *sadi, unsigned long mode,
...
static void sprd_adi_restart_sc9860(struct restart_data *data)
{
	...
	sprd_adi_restart(sadi, data->mode, data->cmd, &wdg);
}
...
static int sprd_adi_probe(struct platform_device *pdev)
{
	...
	if (sadi->data->restart) {
		ret = devm_register_simple_restart_handler(&pdev->dev,
							   sadi->data->restart,
							   sadi);
		if (ret) {
