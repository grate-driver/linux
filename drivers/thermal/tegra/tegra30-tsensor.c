...
static int tegra_tsensor_hw_enable(const struct tegra_tsensor *ts)
{
	...
	/*
	 * Sensors are enabled after reset by default, but not gauging data
...
static int tegra_tsensor_get_temp(void *data, int *temp)
{
	...
	/*
	 * This shouldn't happen with a valid counter, nevertheless
	 * safer to verify the value since it's in a separate register.
	 */
	if (counter == 0xffff) {
...
static int tegra_tsensor_set_trips(void *data, int low, int high)
{
	...
	/*
	 * TSENSOR doesn't trigger interrupt on low temperature breach,
				       ...
				       unsigned int id)
{
	...
	if (FIELD_GET(TSENSOR_SENSOR0_STATUS0_STATE, val) == 5)
		dev_err_ratelimited(ts->dev, "counter overflowed\n");
...
{
	const struct tegra_tsensor *ts = data;
	unsigned int i, div2_state = false;
	bool handled = false;
					   ...
					   unsigned int id)
{
	...
	dev_info_once(ts->dev, "ch%u: hardware shutdown trip set to %dC\n",
		      id, DIV_ROUND_CLOSEST(crit_trip, 1000));

	val = readl_relaxed(tsc->regs + TSENSOR_SENSOR0_CONFIG0);

	/*
	 * Enable DIV2 throttle, emergency shutdown and interrupts for
	 * level 1/2/3 breaches and counter overflow condition.
	 *
	 * Thermal levels:
	 *
	 *     Level 0 = cold
	 *     Level 1 = passive cooling (DVFS)
	 *     Level 2 = passive cooling assisted by hardware
	...
	thresh = readl_relaxed(tsc->regs + TSENSOR_SENSOR0_CONFIG1);
	thresh &= ~TSENSOR_SENSOR0_CONFIG1_TH2;
	thresh |= FIELD_PREP(TSENSOR_SENSOR0_CONFIG1_TH2, hot_trip);
	writel_relaxed(thresh, tsc->regs + TSENSOR_SENSOR0_CONFIG1);

	/* program level3 counter threshold */
	thresh = readl_relaxed(tsc->regs + TSENSOR_SENSOR0_CONFIG2);
	thresh &= ~TSENSOR_SENSOR0_CONFIG2_TH3;
	thresh |= FIELD_PREP(TSENSOR_SENSOR0_CONFIG2_TH3, crit_trip);
	writel_relaxed(thresh, tsc->regs + TSENSOR_SENSOR0_CONFIG2);
...
{
	int err, t1_25C = 0, t2_90C = 0, f1_25C, f2_90C;
	u32 ate_ver = 0, val = 0, i;
	...
	err = tegra_fuse_readl(TEGRA30_FUSE_TSENSOR_CALIB, &val);
	if (err) {
		dev_err(ts->dev, "failed to get calibration data: %d\n", err);
		return err;
	}

	f1_25C = FIELD_GET(TEGRA30_FUSE_TSENSOR_CALIB_LOW, val);
	f2_90C = FIELD_GET(TEGRA30_FUSE_TSENSOR_CALIB_HIGH, val);
	...
	if (f2_90C - f1_25C <= t2_90C - t1_25C) {
		dev_err(ts->dev, "invalid calibration data: %d %d %d %d\n",
			f2_90C, f1_25C, t2_90C, t1_25C);
		return -EINVAL;
	}
	...
	ts->calib.b = t1_25C * 1000000 - ts->calib.a * f1_25C;
	...
	dev_info_once(ts->dev,
		      "calibration: %d %d %d %d ATE ver: %u SoC rev: %u\n",
		      f2_90C, f1_25C, t2_90C, t1_25C, ate_ver,
