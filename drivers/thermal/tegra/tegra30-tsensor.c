...
{
	int err, t1_25C = 0, t2_90C = 0, c1_25C, c2_90C;
	...
	/* calibrated counter values for ~25/90C temperatures */
	c1_25C = FIELD_GET(TEGRA30_FUSE_TSENSOR_CALIB_LOW, val);
	c2_90C = FIELD_GET(TEGRA30_FUSE_TSENSOR_CALIB_HIGH, val);

	/* calibrated temperatures which correspond to the counter values */
	for (i = 0, val = 0; i < 7; i++) {
