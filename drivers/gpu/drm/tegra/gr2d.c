...
static int gr2d_probe(struct platform_device *pdev)
{
	...
	gr2d->rst = devm_reset_control_get_exclusive_released(dev, NULL);
	if (IS_ERR(gr2d->rst)) {
...
static int __maybe_unused gr2d_runtime_suspend(struct device *dev)
{
	...
	reset_control_release(gr2d->rst);
...
static int __maybe_unused gr2d_runtime_resume(struct device *dev)
{
	...
	err = reset_control_acquire(gr2d->rst);
	if (err) {
...
static __maybe_unused int gr2d_resume(struct device *dev)
{
	struct gr2d *gr2d = dev_get_drvdata(dev);
	int err;
...
static const struct dev_pm_ops tegra_gr2d_pm = {
	...
	SET_SYSTEM_SLEEP_PM_OPS(gr2d_suspend, gr2d_resume)
};
