...
static int gr3d_probe(struct platform_device *pdev)
{
	...
	if (gr3d->soc->num_clocks != gr3d->nclocks) {
		dev_err(&pdev->dev, "invalid number of clocks: %u\n",
			gr3d->nclocks);
		return -EINVAL;
	}
...
acquire_reset:
	reset_control_acquire(gr3d->reset);

	if (clk_bulk_prepare_enable(gr3d->nclocks, gr3d->clocks))
		dev_err(dev, "failed to restore clock\n");
