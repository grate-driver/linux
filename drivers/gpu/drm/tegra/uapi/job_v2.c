		       ...
		       struct dma_fence **pfence)
{
	...
	*pfence = dma_fence_get(&job->sched_job.s_fence->finished);
	 ...
	 */
	if (job->out_syncobj)
		drm_syncobj_replace_fence(job->out_syncobj, NULL);
			    ...
			    struct drm_file *file)
{
	...
	err = tegra_drm_lock_reservations(&acquire_ctx, job, user_data);
	if (err)
		goto err_free_job;
	...
	tegra_drm_complete_reservations(&acquire_ctx, job, user_data,
					submit, fence);
