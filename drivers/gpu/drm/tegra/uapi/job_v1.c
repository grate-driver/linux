...
static void tegra_drm_free_job_v1(struct work_struct *work)
{
	...
	tegra_drm_context_v1_put(job_v1->context);
		       ...
		       struct tegra_drm_job **ret_job)
{
	...
	job = kmalloc(job_size, GFP_KERNEL);
	if (!job) {
		err = -ENOMEM;
		goto err_put_syncpt;
	}
		       ...
		       struct drm_file *file)
{
	...
	err = drm_sched_job_init(&job->sched_job,
				 &fpriv->sched_entities[channel->id],
				 NULL);
	if (err) {
	...
	fence = dma_fence_get(&job->sched_job.s_fence->finished);
	context = tegra_drm_context_v1_get(job_v1->context);

	drm_sched_entity_push_job(&job->sched_job,
				  &fpriv->sched_entities[channel->id]);

	drm_syncobj_replace_fence(context->syncobj, fence);
			    ...
			    struct drm_file *file)
{
	...
	/* this essentially maps old submission UAPI into the new */
