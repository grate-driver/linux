...
#define TEGRA_DRM_UAPI_MAX_VERSION	0
...
{
	drm_syncobj_put(context->syncobj);
	kfree(context);
}
...
{
	struct tegra_drm_context_v1 *context =
		container_of(kref, struct tegra_drm_context_v1, refcount);
			 ...
			 struct drm_file *file)
{
	...
	if (submit->uapi_ver > TEGRA_DRM_UAPI_MAX_VERSION) {
		DRM_ERROR("unsupported uapi version %u, maximum is %u\n",
			  submit->uapi_ver, TEGRA_DRM_UAPI_MAX_VERSION);
		return -EINVAL;
	}
			    ...
			    struct drm_file *file)
{
	...
	ret = dma_resv_wait_timeout_rcu(bo->gem.resv, write, true, timeout);
	drm_gem_object_put_unlocked(gem);
		       ...
		       struct drm_file *file)
{
	...
	args->uapi_ver = TEGRA_DRM_UAPI_MAX_VERSION;
