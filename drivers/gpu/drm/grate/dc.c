				    ...
				    struct drm_atomic_state *state)
{
	...
	if (dc->soc->has_legacy_blending) {
		tegra_dc_writel(dc, dc_state->ckey.min, DC_DISP_COLOR_KEY0_LOWER);
		tegra_dc_writel(dc, dc_state->ckey.max, DC_DISP_COLOR_KEY0_UPPER);
	}
...
static int tegra_dc_init(struct host1x_client *client)
{
	...
	dc->syncpt = host1x_syncpt_request(host);
	if (IS_ERR(dc->syncpt)) {
		dev_warn(dc->dev, "failed to allocate syncpoint %ld\n",
			 PTR_ERR(dc->syncpt));
		dc->syncpt = NULL;
	} else {
		host1x_syncpt_associate_device(dc->syncpt, dc->dev);
...
cleanup:
	...
	tegra_drm_client_iommu_detach(drm_client, dc->group, true);
...
static int tegra_dc_suspend(struct tegra_dc *dc)
{
...
static int tegra_dc_resume(struct tegra_dc *dc)
{
	struct device *dev = dc->dev;
	int err;
...
static int tegra_dc_couple(struct tegra_dc *dc)
{
	...
	if (dc->soc->coupled_pm && dc->pipe == 1) {
		...
		dc0 = dev_get_drvdata(partner);
		dc->client.parent = &dc0->client;
...
static int tegra_dc_rpm_suspend(struct device *dev)
{
	struct tegra_dc *dc = dev_get_drvdata(dev);
	...
	return tegra_dc_suspend(dc);
}

static int tegra_dc_rpm_resume(struct device *dev)
{
	struct tegra_dc *dc = dev_get_drvdata(dev);
	...
	return tegra_dc_resume(dc);
}

static const struct dev_pm_ops tegra_dc_pm_ops = {
	SET_RUNTIME_PM_OPS(tegra_dc_rpm_suspend, tegra_dc_rpm_resume, NULL)
};
