			    ...
			    dma_addr_t end)
{
	...
	list_for_each_entry_safe(bo, tmp, victims_list, mm_eviction_entry) {
		...
		tegra_bo_mm_evict_bo(tegra, bo, release, cleanup, true);
	}
}
...
mm_err:
	if (err) {
		...
		tegra_bo_mm_release_victims(tegra, &victims_list, false, true,
					    0, 0);
	} else {
		/*
		 ...
		 * BO range.
		 */
		tegra_bo_mm_release_victims(tegra, &victims_list, false, false,
					    bo->gartaddr,
					    bo->gartaddr + bo->gem.size);

		DRM_DEBUG("%p success iosize %zu gartaddr %pad\n",
...
static void tegra_drm_gart_flush_cache_locked(struct tegra_drm *tegra)
{
	...
	drm_mm_for_each_node_safe(mm, tmp, &tegra->mm) {
		...
		DRM_DEBUG("%p gem_size %zu gartaddr %pad iomap_cnt %u\n",
	...
	drm_mm_for_each_node(mm, &tegra->mm) {
		...
		if (list_empty(&bo->mm_eviction_entry)) {
			...
			*gart_busy |= (i == num_bos);
			...
			DRM_DEBUG("%p pinned gem_size %zu gartaddr %pad iomap_cnt %u\n",
				  bo, bo->gem.size, &bo->gartaddr, bo->iomap_cnt);
		} else {
			DRM_DEBUG("%p cached gem_size %zu gartaddr %pad\n",
				  bo, bo->gem.size, &bo->gartaddr);
		}
	}
...
int tegra_drm_gart_map_optional(struct tegra_drm *tegra, struct tegra_bo *bo)
{
	...
	/*
	 * This will print out a list of mapped BOs if debug is enabled and
	 * nothing more.
	 */
	tegra_drm_job_pre_check_gart_space(tegra, NULL, 0, 0, &gart_busy,
					   gart_security_level);

	/*
	 * This function is only used by tegra_dc_pin() and it's more
	 * optimal to release cache entirely in order to prevent unfortunate
	 * fragmentation of the cache since display framebuffer will stay
	 * pinned for a long time.
	 */
	tegra_bo_mm_release_victims(tegra, &tegra->mm_eviction_list,
				    true, true, 0, 0);
