...
#define SYNCPT(idx)				(host->base_regs + HOST1X_SYNC_OFFSET + HOST1X_SYNC_SYNCPT(idx))
#define SYNCPT_THRESH_CPU0_INT_STATUS(idx)	(host->base_regs + HOST1X_SYNC_OFFSET + HOST1X_SYNC_SYNCPT_THRESH_CPU0_INT_STATUS(idx))
#define SYNCPT_THRESH_CPU0_INT_DISABLE(idx)	(host->base_regs + HOST1X_SYNC_OFFSET + HOST1X_SYNC_SYNCPT_THRESH_INT_DISABLE(idx))
#define SYNCPT_THRESH_CPU0_INT_ENABLE(idx)	(host->base_regs + HOST1X_SYNC_OFFSET + HOST1X_SYNC_SYNCPT_THRESH_INT_ENABLE_CPU0(idx))
#define SYNCPT_INT_THRESH(idx)			(host->base_regs + HOST1X_SYNC_OFFSET + HOST1X_SYNC_SYNCPT_INT_THRESH(idx))

#if HOST1X_HW < 6
#define SYNC_IP_BUSY_TIMEOUT			(host->base_regs + HOST1X_SYNC_OFFSET + HOST1X_SYNC_IP_BUSY_TIMEOUT)
#define SYNC_CTXSW_TIMEOUT_CFG			(host->base_regs + HOST1X_SYNC_OFFSET + HOST1X_SYNC_CTXSW_TIMEOUT_CFG)
#define SYNC_USEC_CLK				(host->base_regs + HOST1X_SYNC_OFFSET + HOST1X_SYNC_USEC_CLK)
#else
#define HV_SYNCPT_PROT_EN			(host->hv_regs + HOST1X_HV_SYNCPT_PROT_EN)
#endif
...
{
	/* optimize code a tad for older hardware that has 32 sync points */
	if (HOST1X_SYNCPTS_NUM == 32)
		return 0;
...
host1x_hw_init_syncpts(struct host1x *host)
{
	...
	/* update host clocks per microsecond */
