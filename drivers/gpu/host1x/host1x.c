...
static int host1x_init_opp_table(struct host1x *host)
{
	 ...
	 * device-tree.  The add_table() errors out if OPP missing in DT.
	 */
...
static void host1x_deinit_opp_table(struct host1x *host)
{
	struct opp_table *opp_table = dev_pm_opp_get_opp_table(host->dev);
...
static int host1x_init(struct host1x *host)
{
	...
	err = host1x_init_opp_table(host);
	if (err)
		return err;
