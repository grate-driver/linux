...
static int rn5t618_i2c_probe(struct i2c_client *i2c)
{
	...
	if (of_device_is_system_power_controller(i2c->dev.of_node))
		priv->sys_off.power_off_cb = rn5t618_power_off;

	priv->sys_off.cb_data = i2c;
	priv->sys_off.restart_cb = rn5t618_restart;
	priv->sys_off.restart_priority = RESTART_PRIO_HIGH;

	ret = devm_register_sys_off_handler(&i2c->dev, &priv->sys_off);
	if (ret) {
		dev_err(&i2c->dev,
