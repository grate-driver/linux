...
static int tegra_core_domain_probe(struct platform_device *pdev)
{
	 ...
	 * Core domain from the on/off callbacks, hence we need to annotate
	 * the lock in order to remove confusion from the lockdep checker
	 * when a nested access happens.
	 */
