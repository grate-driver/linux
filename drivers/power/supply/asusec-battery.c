...
#define ASUSEC_BATTERY_DATA_FRESH		5000
...
static int asusec_battery_refresh(struct asusec_battery_data *priv)
{
	...
	priv->batt_data_ts = jiffies + msecs_to_jiffies(ASUSEC_BATTERY_DATA_FRESH);
				    ...
				    union power_supply_propval *val)
{
	...
	struct power_supply_battery_info battery_info = priv->batt_info;
	int ret;
...
static void asusec_battery_poll_work(struct work_struct *work)
{
	...
	schedule_delayed_work(&priv->poll_work,
			      msecs_to_jiffies(ASUSEC_BATTERY_DATA_FRESH));
}
