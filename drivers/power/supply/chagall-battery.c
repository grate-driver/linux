			     ...
			     u8 reg_addr, u8 value)
{
	...
	if (ret < 0) {
		dev_err(&cg->client->dev, "failed to write registers: %d\n", ret);
		return ret;
	}
					     ...
					     enum led_brightness brightness)
{
	...
	chagall_write_reg(cg, CHAGALL_REG_AMBER_LED, brightness);
}
					     ...
					     enum led_brightness brightness)
{
	...
	chagall_write_reg(cg, CHAGALL_REG_WHITE_LED, brightness);
}
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP_MIN:
		if (!cg->batt_info || cg->batt_info->temp_min == INT_MIN)
			return -ENODATA;
	...
	case POWER_SUPPLY_PROP_TEMP_MAX:
		if (!cg->batt_info || cg->batt_info->temp_max == INT_MAX)
			return -ENODATA;
