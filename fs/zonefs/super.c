			      ...
			      void *data)
{
	...
	/*
	 * At this point, we know the zone write pointer offset for sequential
	 * zones and the zone new condition. We can return early if we are
	 * dealing with a conventional zone (no write pointer) or if the zone
	 * condition has changed to OFFLINE or READ-ONLY.
	 */
	if (zi->i_ztype == ZONEFS_ZTYPE_CNV ||
	...
	/*
	 * An IO error or an external action on the device caused the inode
	 * size and the zone write pointer offset to differ.
	 * 1) The inode size is below the zone WP: data was writen at the end of
	 *    the file. This can happen in the case of a partial write failure
	 *    of a large DIO needing several BIOs and/or write requests to be
	 *    processed. Since the zone WP position relative to the zone start
	 *    always indicates the amount of data written (and readable) in the
	 *    zone, simply increase the inode size to reflect the amount of
	 *    readable data that was writen.
	 * 2) The inode size is over the zone WP: this can only happen if the
	 *    device write cache is enabled and and IO error occured during a
	 *    device cache flush after we got successful write completions.
	 *    Other possibilities are (a) an external corruption, e.g. an
	 *    application reset the zone directly, and (b) if the device has a
	 *    serious problem (e.g. firmware bug). For this case, act based on
	 *    mount options: remount readonly, set the zone readonly or set the
	 *    zone offline.
	 */
	...
	if (isize > wp_ofst) {
		...
		} else if (sbi->s_mount_opts & ZONEFS_MNTOPT_ERRORS_REPAIR) {
			action = ", file repaired";
		} else if (sbi->s_mount_opts & ZONEFS_MNTOPT_ERRORS_ZRO) {
			action = ", file changed to read-only";
			zone->cond = BLK_ZONE_COND_READONLY;
		} else if (sbi->s_mount_opts & ZONEFS_MNTOPT_ERRORS_ZOL) {
			action = ", file read/write access disabled";
			zone->cond = BLK_ZONE_COND_OFFLINE;
		}
		...
		if (action)
			zonefs_warn(sb, "inode %lu corruption detected%s\n",
				    inode->i_ino, action);
	}
 ...
 */
static void zonefs_io_error(struct inode *inode)
{
	struct zonefs_inode_info *zi = ZONEFS_I(inode);
	struct super_block *sb = inode->i_sb;
	struct zonefs_sb_info *sbi = ZONEFS_SB(sb);
...
{
	if (error) {
		zonefs_io_error(file_inode(iocb->ki_filp));
		return error;
	}
