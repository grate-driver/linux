	...
	for (i = 0; i < PTRS_PER_PTE; i++)
		set_pte_at(&init_mm, KASAN_SHADOW_START + i*PAGE_SIZE,
			&kasan_zero_pte[i], pfn_pte(
				...
				__pgprot(L_PTE_MT_WRITETHROUGH |
					_L_PTE_DEFAULT |
					 L_PTE_DIRTY |
					 L_PTE_XN)));
...
pte_t * __meminit kasan_pte_populate(pmd_t *pmd, unsigned long addr, int node)
{
	...
	if (pte_none(*pte)) {
		...
		if (!p)
			return NULL;
		entry = pfn_pte(virt_to_pfn(p), __pgprot(L_PTE_MT_WRITETHROUGH |
							_L_PTE_DEFAULT |
							 L_PTE_DIRTY |
							 L_PTE_XN));
...
void __init kasan_init(void)
{
	...
	pr_info("Kernel address sanitizer +\n");
	...
	set_pgd(&tmp_page_table[pgd_index(KASAN_SHADOW_START)],
	 ...
	 * so we need mapping.
	 *
	 * 2.PKMAP_BASE ~ PKMAP_BASE+PMD_SIZE's shadow and
	 * MODULES_VADDR ~ MODULES_END's shadow is in the same PMD_SIZE,
	 * so we cant use kasan_populate_zero_shadow.
	 *
	 */
	create_mapping(
