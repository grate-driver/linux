...
static inline struct asusec_info *asusec_cell_to_ec(struct platform_device *pdev)
{
	return dev_get_drvdata(pdev->dev.parent);
}

static inline int asusec_register_notifier(struct asusec_info *ec,
					   struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&ec->notify_list, nb);
}

static inline int asusec_unregister_notifier(struct asusec_info *ec,
					     struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&ec->notify_list, nb);
}
